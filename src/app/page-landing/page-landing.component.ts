...
export class PageLandingComponent implements OnInit {
        ...
        .subscribe(
          (data: string) => {
            ...
            this.http.get(`http://ip-api.com/json/`).subscribe(data => {
              this.selfInfo = data;
              this.selfInfo.flagClass = `flag-icon flag-icon-${this.selfInfo.countryCode.toLowerCase()}`;
              this.currentCountry = this.countries.filter(
                x =>
                  x.code.toLowerCase().trim() ===
                  this.selfInfo.countryCode.toLowerCase().trim()
              )[0];
              this.currentCountry.currency =
                qs.currency || this.currentCountry.currency;
              if (this.selfInfo) {
                const country =
                  countriesList.countries[this.selfInfo.countryCode];
                if (
                  country &&
                  country.languages &&
              ...
              // TODO: delete this check for portugal language, after portuguese version of the web page will be done
              if (this.selfInfo.lang === 'pt') {
                this.http
                  .get(`assets/lang/en/strings.ini`, { responseType: 'text' })
                  // tslint:disable-next-line:no-shadowed-variable
                  .subscribe((data: string) => {
                    this.loadData(data);
                  });
  ...
  loadData(data) {
    ...
    this.http
      .get(
        `https://uinames.com/api/?amount=10&region=${this.selfInfo.country}&ext`
      )
      // tslint:disable-next-line:no-shadowed-variable
      .subscribe(data => {
        this.allPeople = data;
        const countriesTemp = _.shuffle(
          cities.getCities(this.selfInfo.country).filter(x => x.length < 15)
        );
